...
- **Irie** — unified MIR + MachineCode scaffolding; not yet wired into the main compiler
  ...
  - `Passes/` — all generic passes plus pass infrastructure (`Pass`, `PassManager`, `CompilationContext`, `MirFunctionPass`, `MirFunctionAnalysis`). Pipeline order in `iriec` is (`ReturnMergePass` runs first, before `AbiLoweringPass`: when a function has ≥2 `core.return` blocks it merges them into one common return block — each return becomes `cf.br retblock(%v)` with the value as a block parameter — so the return epilogue is emitted once; the SimplifyCFG `tailMergeBlocksWithSimilarFunctionTerminators` analogue, merging unconditionally with no cost model. Where merging a *trivial* tail did not pay off, the post-RA `MOS6502TailDuplicationPass` rebalances it back (see below) — same division of labour as llvm-mos's SimplifyCFG-merge + TailDuplicate):
    ...
    8. *Target-supplied post-RA passes* — added by `Target.AddPostRegisterAllocationPasses(pm)`. For MOS6502: `MOS6502AddressingModeSelectorPass` (refines `mos6502.adc` → `mos6502.adc.zp` / `.imm` etc. based on concrete physreg operands). `Target.AddFinalPasses(pm)` then runs (after RegisterScavenging) `MOS6502BlockPlacementPass` (fall-through layout) followed by `MOS6502TailDuplicationPass` — the llvm-mos `tailduplication` analogue that rebalances trivial shared return tails left by `ReturnMergePass`/the branch-folder: it clones a lone `rts` back into the predecessors that still `jmp` to it (cost limit ≤1 instr, mirroring `TailDuplicator::shouldTailDuplicate` at -Os). It runs *after* block-placement — unlike llvm-mos's standalone pre-placement pass — because our block-placement lacks integrated tail-dup, so by then only the `jmp`s placement couldn't turn into free fall-throughs remain (running before placement would clone over jmps about to be dropped, growing code — caught by the `OutOfRangeBranch` byte-size golden).
