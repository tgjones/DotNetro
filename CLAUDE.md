...
- **Irie** — IR, CodeGen, and MachineCode scaffolding; not yet wired into the main compiler
  ...
  - `CodeGen/` — LLVM CodeGen-style layer; sits between IR and MachineCode; starts in SSA with generic opcodes, lowered to target-legal non-SSA by passes
    ...
    - `Passes/MachineFunctionPass` — abstract pass base; `MachineFunctionPassManager` runs passes over a module or function
      ...
      - `LegalizerPass` — widens/narrows types to target-legal ones using `LegalizerInfo`; e.g. MOS6502 legalizes `GenericAdd i32` → 4× `GenericAddCarry i8` chain
      - `InstructionSelectorPass` — selects target opcodes using `InstructionSelector`; e.g. MOS6502 maps `GenericAddCarry` → `CLC`+`ADC_ZeroPage` / `ADC_ZeroPage`
    ...
    - `MachineIRBuilder` — insertion-point-based builder helper; tracks `(block, index)` and provides typed `Build*` methods
  ...
  - `Target/MOS6502/` — 6502 target; `MOS6502Opcode` constants equal the 6502 byte values; `MOS6502InstructionInfo` descriptor table with `GetDisplayName` returning `"{Mnemonic}_{Mode}"` strings; `MOS6502AssemblyWriter` / `MOS6502AssemblyParser` for traditional `$FF` / `#$FF` assembly syntax
    ...
    - `MOS6502CallLowering` — CC_MOS convention: i8 bytes assigned in order A, X, RC2, RC3, … (LSB-first for multi-byte values); `LegalizerInfo` legalizes `GenericAdd i32 → NarrowScalar`; `InstructionSelector` uses `_mergeMap` to eliminate Merge/Unmerge pairs and select ADC chains
