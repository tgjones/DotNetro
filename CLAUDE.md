...
- **Irie** — unified MIR + MachineCode scaffolding; not yet wired into the main compiler
  ...
  - `Passes/` — all generic passes plus pass infrastructure (`Pass`, `PassManager`, `CompilationContext`, `MirFunctionPass`, `MirFunctionAnalysis`). Pipeline order in `iriec` is (`ReturnMergePass` runs first, before `AbiLoweringPass`: when a function has ≥2 `core.return` blocks and a multi-byte return, it merges them into one common return block — each return becomes `cf.br retblock(%v)` with the value as a block parameter — so the return epilogue is emitted once; the SimplifyCFG `tailMergeBlocksWithSimilarFunctionTerminators` analogue, gated to `>8`-bit returns since Irie has no tail-duplication to rebalance trivial tails):
