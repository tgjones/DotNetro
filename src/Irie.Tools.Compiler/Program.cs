...
rootCommand.SetAction(parseResult =>
{
    ...
    // Lower materialized arith.select into diamonds BEFORE the legalizer, so the
    // legalizer sees the resulting cf.cond_br and owns all compare narrowing
    // uniformly (a wide cmpi feeding a select would otherwise escape narrowing,
    // since its cond_br does not exist yet).
