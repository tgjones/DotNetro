...
## STATUS — last updated 2026-06-11 (branch `irie-close-gap`)

**To resume: this is a partially-executed plan. Steps 1–9 are done — all 21 lit
tests are ported to `Lit/Mir/` and green on `--mir` (Debug + Release). Only
step 10 (flip the default + delete the old path) remains. Read this section,
then continue at "What's left" below.**
...
**Step 9 — DEFERRED groups (5 tests still legacy-only).** Each remaining test
...
| ~~**Cross-call value preservation**~~ — **DONE (2026-06-10), caller-side `.bss` interim (static-stack-alloc-plan Layer 1).** Implemented in the *translator* rather than as a MIR pass: `IlToMirTranslator` now computes which locals **and parameters** are live across a call (`ComputeCrossCallLive`: combined local+param liveness fixpoint, then a per-block backward scan flagging every variable live immediately after a `call`/`callvirt`/`newobj`) and forces those into FrameSlots — locals via the existing struct/address-taken path, parameters via a new entry-block spill (`mem.store` the incoming register vreg) + `mem.load` on each `ldarg`. Cross-call values therefore live in `.bss` and never sit in a register across the call, so **no `CallerSavedScratch`/Layer-0 change was needed** and the RA-level spill path is untouched. Did *not* migrate to the llvm-mos callee-saved-register model (RA prefers RC20..RC31 + prologue/epilogue save + convention-clean runtime helpers + reentrancy detection) — that remains the documented follow-up ([static-stack-alloc-plan.md](static-stack-alloc-plan.md) implementation-order step 3), **which must first remove this translator-side routing** (`ComputeCrossCallLive`, the cross-call classification branches, and the parameter entry-spill / memory-`ldarg` path) so cross-call values reach the RA as ordinary vregs instead of being pre-forced into `.bss`. Two bugs fixed en route: (1) latent `TranslateStloc` for a FrameSlot **primitive** local used the `_localTypes` I16 placeholder → `mem.store.i16` of an i32 value (now uses the local's natural CLR width, matching `TranslateLdloc`); (2) `MOS6502InstructionSelector`'s pointer-setup cache (`_currentPointerKey`) was not invalidated across a `jsr` — a slot pointer set up before a call was reused after, but the call clobbers the shared `$zp0/$zp1` indirect pair (the long-anticipated "calls must invalidate" gap). Lit test: `Lit/Mir/CallNestedMethods.cs` (un-held). | ~~CallNestedMethods~~, and all heap/class groups |
| ~~**ManagedHeap_Alloc + vtables + callvirt / indirect dispatch**~~ — **DONE (2026-06-11).** All 5 class tests ported to `Lit/Mir/` and green. Translator gained: `newobj` (→ `call ManagedHeap_Alloc(size, &Vtable_<T>)` + `call ctor`), `callvirt` (non-virtual → direct call; virtual → load vtable ptr at `this-2`, load fptr at `vtable+slot*2`, `call.indirect`), `ldfld`/`stfld` of reference-type fields, `sizeof`/`conv.i`/`stind.i`/`dup`, reference-type → i16 in `ToIRType` (incl. `System.Object`), eager **vtable layout** computed from metadata (stops at the root so BCL virtuals get no slots → indices are metadata-only, no used-set fixpoint), vtable globals (`DataSymbolRef` per slot), and **static-constructor** execution (`.cctor` of static-field owners run from `__entry` before Main — needed for `ManagedHeap.s_heapPtr = 0x1000`). **ManagedHeap.Alloc kept in C#** and translated through the pipeline (plan open-question 3). Two backend bugs fixed en route, both general (not class-specific): (1) `MOS6502CallLowering` materialized each arg's bytes then immediately pinned it to a physreg, so a `mem.symbol` arg in a non-first slot (its hard-$a `lda.imm`) collided with an earlier arg already pinned to $a → the RA could not colour ANY `newobj` (even `var c = new X()`); fix splits arg setup into two phases (materialize all byte values, then pin) so the symbol's lda runs before $a is occupied. (2) `RegisterScavengingPass` threw `NotImplementedException` when no scratch GPR was free; `ManagedHeap.Alloc`'s i16 pressure is the first case to reach it, so the documented Phase-4 minimal save/restore is now implemented (save a live GPR into a dead zero-page slot via `TargetRegisterInfo.GetEmergencyScratchSaveSlots`, do the copy, restore). The translator's earlier cross-call FrameSlot routing already handles cross-call values; the RA also spills cross-call eval-stack temps fine, so `newobj` needed no extra translator-side spilling. Lit tests: `Lit/Mir/{UseClass,UseNestedClass,UseNestedClassWithConstructors,CallInstanceMethodOnClass,CallOverriddenMethodOnClass}.cs`, `Lit/CodeGen/MOS6502/CallFuncSymbolArg-AbiLowering.irie`, updated `RegisterScavengingTests.AllGprsLive_EmergencySavesGprToZeroPage`. | UseClass, UseNestedClass, UseNestedClassWithConstructors, CallInstanceMethodOnClass, CallOverriddenMethodOnClass |

All 5 deferred tests are now ported and green. Step 9 is complete.
